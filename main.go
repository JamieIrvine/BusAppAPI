...
func convertLiveBuses(buses []TranslinkLiveBus) []LiveBusOutput {
	...
	result := make([]LiveBusOutput, 0)

	for _, b := range buses {

		// Parse using the correct layout
		var dateStr string
...
func main() {
	...
	r.GET("/live-buses", func(c *gin.Context) {
		...
		if serviceNumberParam != "" {
			...
			allBuses := converted
			converted = make([]LiveBusOutput, 0)

			for _, b := range allBuses {
				if strings.ToLower(b.ServiceNumber) == strings.ToLower(serviceNumberParam) {
